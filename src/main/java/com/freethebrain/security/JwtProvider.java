...
public class JwtProvider {
	...
	// Date refreshTokenExpiryDate = new Date(now.getTime() +
	// appProperties.getAuth().getRefreshTokenExpirationMsec());

	String token = Jwts.builder().setSubject(Long.toString(userPrincipal.getId()))// use userId for identify
										      // currentUser
		.setIssuedAt(new Date())// when token is created
		.setExpiration(accessTokenExpiryDate).signWith(this.getSigningKey(), SignatureAlgorithm.HS512)
		.compact();
	return token;
    }
	...
	String token = Jwts.builder().setSubject(Long.toString(userPrincipal.getId()))// use userId for identify
										      // currentUser
		.setIssuedAt(new Date())// when token is created
		.setExpiration(refreshTokenExpiryDate).signWith(this.getSigningKey(), SignatureAlgorithm.HS512)
		.compact();
	return token;
    }

    public Claims extractTokenClaims(String token) {
	Claims claims = Jwts.parserBuilder().setSigningKey(this.getSigningKey()).build().parseClaimsJws(token)
		.getBody();
	return claims;
