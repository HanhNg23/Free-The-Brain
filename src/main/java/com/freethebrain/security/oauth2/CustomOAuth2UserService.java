...
import com.freethebrain.security.oauth2.user.OAuth2UserInfo;
import com.freethebrain.security.oauth2.user.OAuth2UserInfoFactory;
...
public class CustomOAuth2UserService extends DefaultOAuth2UserService { // this class has implements OAuth2UserService
	...
	// In here, means that user email found in system so just let them update
	// existing User
	if (userOptional.isPresent()) {
	    user = userOptional.get();// If a value is present, returns the value, otherwise throws
	...
	User user = User.builder().provider(AuthProvider.valueOf(oauth2UserRequest.getClientRegistration().toString()))
		.providerId(oauth2UserInfo.getId()).accountName(oauth2UserInfo.getAccountName())
		.email(oauth2UserInfo.getEmail()).imageUrl(oauth2UserInfo.getImageurl()).build();
